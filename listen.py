...
def sendemail(msg_text, subject, recipients=None, attachments=[]):
    ...
    if recipients is None:
        try:
            recipients_file = email_conf.get('Recipients File')
            recipients = []
            with open(recipients_file, "r") as f:
                for anaddress in f.readlines():
                    anaddress = anaddress.strip()
                    if anaddress != "" and anaddress[0] != "#":
                        recipients.append(anaddress)
        except:
...
def process_gcn(payload, root):
    ...
    try:
        ...
        logger.error("Error sending messasge to Slack.")
